...
void setup()
{
  // Enable USB CDC for ESP32-S3 (USB串口)
  #if ARDUINO_USB_CDC_ON_BOOT
  Serial.setTxTimeoutMs(0);  // 避免USB串口阻塞
  #endif
  
  // Enable serial port
  Serial.begin(115200);
  
  // Wait for USB serial connection (ESP32-S3)
  #if ARDUINO_USB_CDC_ON_BOOT
  ...
  Serial.println("=== ATS-MINI USB Serial Active ===");
  Serial.println("If you see this, USB CDC is working!");
  Serial.flush();
  
  // Add extra delay to ensure message is sent
  delay(500);
  ...
  Serial.println("=== ATS-MINI System Startup Complete ===");
  Serial.printf("Version: %s\n", getVersion(true));
  Serial.println("Serial output active at 115200 baud");
  Serial.println("");
  Serial.println("To activate GalGame:");
  Serial.println("1. Use menu item 'GalGame' on device, OR");
  Serial.println("2. Send serial command: :GG ENTER");
  Serial.println("");
  Serial.println("GalGame remote commands:");
  Serial.println("  :GG ENTER    - Start GalGame");
  Serial.println("  :GG STATUS   - Check status");
  Serial.println("  :GG LIST     - List projects");
  Serial.println("  :GG LEAVE    - Exit GalGame");
  Serial.println("==========================================");
}
...
void loop()
{
  ...
  // Heartbeat log every 30 seconds to confirm system is running
  static uint32_t lastHeartbeat = 0;
  if(currentTime - lastHeartbeat > 30000) {
    Serial.printf("[%lu] System running, GalGame: %s\n", 
                  currentTime/1000, galgameActive() ? "ACTIVE" : "idle");
    Serial.flush();
  ...
  // If GalGame app is active, divert input & skip radio logic
  if(galgameActive()) {
    // Map encoder and button events
    int dir = encoderCount; encoderCount = 0;
    galgameEncoder(dir, pb1st.wasClicked||pb1st.wasShortPressed, pb1st.isLongPressed, pb1st.wasShortPressed);
    galgameLoop();
    // Prevent display sleep & command timeout while in game
    elapsedSleep = elapsedCommand = currentTime;
    delay(5);
    return; // Skip rest of radio loop
  }
